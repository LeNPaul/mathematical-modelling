...
%FullPDE(0.0001,1.3,10,0.6,0.00015,1.5,12,1.2,0.0,1,0,200,600,50,0,200,0)
...
DnR = dN*dt/(dx^2); %heat constant and the r term for ncells
DnR2 = 1-2*DnR; 

DtR = dT*dt/(dx^2); %Heat constant and the r term for tcells
DtR2 = 1-2*DtR;
...
%dirichlet boundary conditions. Function is set to 0 at boundary
ncells(1,1) = 0;
ncells(MeshSp,1)=0;
tcells(1,1) = 0;
tcells(MeshSp,1)=0;
...
%dim = [0.3 0.3 0.3 0.3];
%str = {'Parameters','D_N=0.0001, D_T=0.00015','r_n=1.3, r_t=20, K_N=10',' K_T=12, b_{nt}=0.5, b_{tn}=1.1'};
